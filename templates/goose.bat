...
:: (Claude Code runs as node/claude too). PATH-SCOPED to THIS deployment
:: (2026-08-03): the old loose '*mcp_gmail.js*'/'*mcp_server.js*' patterns
:: matched the INVOICER's Gmail MCP + Claude Desktop MCP too, so every PRECRIME
:: launch silently killed the Leedz extension's :7000 Gmail server.
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_Process | Where-Object { ($_.Name -in 'node.exe','claude.exe','goose.exe') -and $_.CommandLine -and ($_.CommandLine -like '*%~dp0server\mcp\mcp_server.js*' -or $_.CommandLine -like '*%~dp0server\mcp\mcp_gmail.js*' -or (($_.CommandLine -like '*--print*' -or $_.CommandLine -like '*--no-session*') -and $_.CommandLine -like '*precrime*')) } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"
...
:: sessions and the user's Claude Code (also node.exe) must survive. PATH-SCOPED
:: to THIS deployment (2026-08-03) so the INVOICER's mcp_gmail.js/mcp_server.js
:: survive a PRECRIME launch; worker recipes are matched by their precrime-*.yaml
:: temp-file names.
