...
<!-- Your rate card, in plain bullets. Two jobs:
     1. The outreach drafter quotes from it.
     2. Payment-history importers (e.g. Square) read a payment AMOUNT and infer
        WHAT it was for, so they can predict when to contact that buyer again.
     Fill in the rate lines first, then write the amount bands that follow from
     them. Bands must not overlap and should cover every amount you ever take. -->

- **Hourly rate:** [$X/hr]
- **Commission (repeat customer):** [$X]
- **Commission (typical):** [$X]
- **Normal tip range:** [$X to $Y]
- **Card surcharge:** [X%]
- [Deposit / payment terms]

- [$A-$B] -> tip
- [$C-$D] -> typically a commission
- [$E+] -> multi-hour event; hours ~= amount / [surcharge] / [hourly rate]
