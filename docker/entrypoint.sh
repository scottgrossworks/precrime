...
# Wire up API keys from env vars into Hermes auth
...
# /db is container-native ext4 -- writes work fine here.
mkdir -p /db
...
# On exit: sync database back to the Windows volume so changes persist.
sync_db() {
...
if [ -f "/precrime/rss/rss-scorer-mcp/rss_config.json" ]; then
  ...
  echo ">>> This means hermes.bat was run from the wrong folder."
  echo ">>> Run hermes.bat from your deployment folder (e.g. PHOTOBOOTH\\precrime), not from PRECRIME."
fi

echo ">>> Launching Hermes..."
cd /precrime

# Do NOT use exec -- shell must stay alive so EXIT trap fires on shutdown
hermes chat
